            ...
            foreach (string upload in Request.Files)
            {
                ...
                else if (file.ContentLength > 0)
                {
                        ...
                        || (Path.GetExtension(file.FileName).ToLower().Contains(".pdf")))
                    {
                        ...
                        {
                            file.SaveAs(Server.MapPath("~/Content/Uploads/" + orgName + "/Products/" + NewFileName));  //" + Request.Form["orgname"]+ "
                            // save products to db
                        }
                        else if (upload.ToString().Contains("_vCard_"))
                        {
                            file.SaveAs(Server.MapPath("~/Content/Uploads/" + orgName + "/VCards/" + NewFileName));
                            // save visiting card to db
                        }
                    }
                }
            }
